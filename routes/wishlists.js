...
const { getWishlistsCollection } = require("../db");
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!item) return res.status(404).json({ error: "Wishlist not found" });
    res.json(item);
  } catch (err) {
    console.error(err);
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!result.value) return res.status(404).json({ error: "Wishlist not found" });
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: "Failed to patch wishlist" });
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.deletedCount === 0) return res.status(404).json({ error: "Wishlist not found" });
